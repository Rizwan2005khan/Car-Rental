      ...
      <form onSubmit={onSubmitHandler} className='flex flex-col gap-5 text-gray-500 text-sm mt-6 max-w-xl'>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col w-full">
            ...
            <input type="number" placeholder='300' required className="px-3 py-2 mt-1 border border-borderColor rounded-md outline-none" value={car.pricePerDay} onChange={e => setCar({...car, pricePerDay: e.target.value})}/>
          </div>
          <div className="flex flex-col w-full">
        ...
        <div className="flex flex-col w-full">
          ...
          <button className='flex items-center gap-2 px-4 py-2 mt-4 bg-primary text-white rounded-md font-medium w-max cursor-pointer'>
            <img src={assets.tick_icon} alt="" />
            {isLoading ? 'Listening...' : ' List Your Car'}
          </button>
      </form>      
    </div>
  )
